...
1. learning a neural network to predict the original image `$\pmb{x}_0$` (the ground truth)
2. the source noise `$\pmb{\epsilon}_0$`
3. or the *score* of the image at an arbitrary noise level `$\nabla log\,p(\pmb{x}_t)$`
