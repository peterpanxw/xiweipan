...
{{<figure src="/figures/blogFigs/ddpmPython/training_algorithm.png" caption="Figure 2: Complete training procedure of the DDPM (Ho et al., 2020)." width="500">}}
...
where `$\pmb{\epsilon}_\theta(\pmb{x}_t,t)\right)$` is parameterized by a neural network (e.g., a U-Net) that is optimized to match the noise term `$\pmb{\epsilon}$` in Equation `$\eqref{eq5}$` in the reverse sampling stage.

Fig. 3 shows the specific pseudocode of the **sampling** procedure of the DDPM.
{{<figure src="/figures/blogFigs/ddpmPython/sampling_algorithm.png" caption="Figure 3: Complete sampling procedure of the DDPM (Ho et al., 2020)." width="500">}}
