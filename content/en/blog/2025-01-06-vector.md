...
Two important theorems are associated with the concept of "curl". Green's Theorem <u>relates a line integral round a closed curve `$C$` to an area integral over the <font color=Blue>plane region</font> `$A$` enclosed by `$C$`.</u> The general form is given by
