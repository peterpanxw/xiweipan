...
here `$\alpha_i=\sigma_i^2/\sigma_L^2$` is the step size and `$\pmb{s}_{\pmb{\theta}}(\pmb{x}_t,\sigma_i)$` denotes the score estimator for noise level `$\sigma_i$`. Iteration over `$t$` is repeated sequentially for each `$\sigma_i,\ i=1,\cdots,L$`.

Specific algorithm regarding the *Annealed Langevin dynamics* is shown in Fig. 2. The standard Gaussian `$\pmb{z}_t$` which serves as added noise is equivalent to the notation`$\pmb{\epsilon}_t$` we used here.
{{<figure src="/figures/blogFigs/diffusionModel/diffusion_fig6.png" caption="Figure 2: Algorithm on Annealed Langevin dynamics (Song and Ermon, 2019)." width="600">}}
