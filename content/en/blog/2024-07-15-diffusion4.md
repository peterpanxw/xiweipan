...
The score function defines a vector field over the entire space that data `$\pmb{x}$` inhabits, pointing towards the modes (as shown in the right panel of Fig. 1). So, by learning the score of true data distribution, samples can be generated by starting at any arbitrary point in the same space and then iteratively following the direction to finally reach a mode. This sampling procedure is known as **Langevin dynamics** defined by Equation `$\eqref{eq1}$`.
...
<font color=RoyalBlue>We should realize that we generally have no access to the ground truth `$p(\pmb{x})$` in the Langevin equation.</font> Fortunately, there are two cheap and dirty ways to approximate the ground truth score function.

<mark>**Explicit Score Matching.**</mark> Given a dataset `$\chi={\pmb{x}_1,\cdots,\pmb{x}_M}$`, the solution one may came up with is to consider the classical kernal density estimation by defining a distribution
`$$q(\pmb{x})=\frac{1}{M}\sum_{m=1}^M\frac{1}{h}K\left(\frac{\pmb{x}-\pmb{x}_m}{h}\right), \tag{8} \label{eq8}$$`
where `$h$` is a certain *hyperparameter* for the kernal function `$K( \bigcdot )$`, and `$\pmb{x}_m$` represents the `$m$`-th sample in the training set.

Equation `$\eqref{eq8}$` indicates that the sum of all individual kernals yields the overall **kernal density estimate** `$q(\pmb{x})$`. Therefore, `$q(\pmb{x})$` is at best an approximation to the unknown true data distribution `$p(\pmb{x})$`. <font color=Chocolate>And we can learn `$\pmb{s}_{\pmb{\theta}}$` based on `$q(\pmb{x})$` as a compromise.</font> This leads to the following definition of the loss function which can be used to train a NN.
> The **explicit score matching** loss is
> `$$J_{\mathrm{ESM}}(\pmb{\theta})\overset{\mathrm{def}}{=}\mathbb{E}_{q(\pmb{x})}\left[\|\pmb{s}_{\pmb{\theta}}(\pmb{x})-\nabla_{\pmb{x}}log\,q(\pmb{x})\|^2\right] \tag{9} \label{eq9}$$`

Substituting Equation `$\eqref{eq8}$` into Equation `$\eqref{eq9}$`, the loss is finally derived by
`\begin{align}
J_{\mathrm{ESM}}(\pmb{\theta})&\overset{\mathrm{def}}{=}\mathbb{E}_{q(\pmb{x})}\left[\|\pmb{s}_{\pmb{\theta}}(\pmb{x})-\nabla_{\pmb{x}}log\,q(\pmb{x})\|^2\right]\\
&= \int\|\pmb{s}_{\pmb{\theta}}(\pmb{x})-\nabla_{\pmb{x}}log\,q(\pmb{x})\|^2\left[\frac{1}{M}\sum_{m=1}^M\frac{1}{h}K\left(\frac{\pmb{x}-\pmb{x}_m}{h}\right)\right]\,\mathrm{d}\pmb{x}\\
&= \frac{1}{M}\sum_{m=1}^M\int\frac{1}{h}\|\pmb{s}_{\pmb{\theta}}(\pmb{x})-\nabla_{\pmb{x}}log\,q(\pmb{x})\|^2K\left(\frac{\pmb{x}-\pmb{x}_m}{h}\right)\,\mathrm{d}\pmb{x}.
\end{align}`
...
<mark>**Denoising Score Matching.**</mark> 
