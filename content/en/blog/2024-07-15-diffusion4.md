...
<font color=RoyalBlue>The **training** procedure of a score-based model is typically achieved by minimizing the DSM loss function.</font> Given a training dataset `$\left\{\pmb{x}^{(m)}\right\}_{m=1}^M$`, the optimization goal is
`\begin{align}
...
\approx\mathop{\arg\max}\limits_{\pmb{\theta}}\frac{1}{2M}\sum_{m=1}^M\|\pmb{s}_{\pmb{\theta}}\left(\pmb{x}^{(m)}+\sigma\pmb{\epsilon}^{(m)}\right)+\frac{\pmb{\epsilon}^{(m)}}{\sigma}\|^2,\quad\pmb{\epsilon}^{(m)}\sim\mathcal{N}\left(0,\mathrm{\pmb{I}}\right).
\end{align}`
