...
> `\begin{align}
J_{\mathrm{ESM}}(\pmb{\theta})&\overset{\mathrm{def}}{=}\frac{1}{2}\mathbb{E}_{p(\pmb{x})}\left[\left\|\pmb{s}_{\pmb{\theta}}(\pmb{x})-\nabla_{\pmb{x}}log\,p(\pmb{x})\right\|^2\right]\\
&\approx\frac{1}{2}\mathbb{E}_{q_h(\pmb{x})}\left[\left\|\pmb{s}_{\pmb{\theta}}(\pmb{x})-\nabla_{\pmb{x}}log\,q_h(\pmb{x})\right\|^2\right]. \tag{9} \label{eq9}
\end{align}`

Substituting Equation `$\eqref{eq8}$` into Equation `$\eqref{eq9}$`, the loss is finally derived by
`\begin{align}
J_{\mathrm{ESM}}(\pmb{\theta})&\overset{\mathrm{def}}{=}\mathbb{E}_{p(\pmb{x})}\left[\left\|\pmb{s}_{\pmb{\theta}}(\pmb{x})-\nabla_{\pmb{x}}log\,p(\pmb{x})\right\|^2\right]\\
&\approx \int\left\|\pmb{s}_{\pmb{\theta}}(\pmb{x})-\nabla_{\pmb{x}}log\,q_h(\pmb{x})\right\|^2q_h(\pmb{x})\,\mathrm{d}\pmb{x}\\
&= \frac{1}{M}\sum_{m=1}^M\int\frac{1}{h}\left\|\pmb{s}_{\pmb{\theta}}(\pmb{x})-\nabla_{\pmb{x}}log\,q_h(\pmb{x})\right\|^2K\left(\frac{\pmb{x}-\pmb{x}^{(m)}}{h}\right)\,\mathrm{d}\pmb{x}.
\end{align}`
...
`$$J_{\mathrm{DSM}}(\pmb{\theta})\overset{\mathrm{def}}{=}\mathbb{E}_{q(\pmb{x},\pmb{x}^\prime)}\left[\frac{1}{2}\|\pmb{s}_{\pmb{\theta}}(\pmb{x})-\nabla_{\pmb{x}}log\,q(\pmb{x}|\pmb{x}^\prime)\|^2\right]. \tag{11} \label{eq11}$$`
...
`$$J_{\mathrm{DSM}}(\pmb{\theta})=\mathbb{E}_{q(\pmb{x}^\prime)}\left[\frac{1}{2}\|\pmb{s}_{\pmb{\theta}}(\pmb{x}^\prime+\sigma\pmb{\epsilon})+\frac{\pmb{\epsilon}}{\sigma}\|^2\right].$$`
...
> `$$J_{\mathrm{DSM}}(\pmb{\theta})=\mathbb{E}_{p(\pmb{x})}\left[\frac{1}{2}\|\pmb{s}_{\pmb{\theta}}(\pmb{x}+\sigma\pmb{\epsilon})+\frac{\pmb{\epsilon}}{\sigma}\|^2\right]. \tag{12} \label{eq12}$$`
...
`\begin{align}
\pmb{\theta}^\ast &= \mathop{\arg\max}\limits_{\pmb{\theta}}\mathbb{E}_{p(\pmb{x})}\left[\frac{1}{2}\|\pmb{s}_{\pmb{\theta}}(\pmb{x}+\sigma\pmb{\epsilon})+\frac{\pmb{\epsilon}}{\sigma}\|^2\right]\\
&\approx\mathop{\arg\max}\limits_{\pmb{\theta}}\frac{1}{2M}\sum_{m=1}^M\|\pmb{s}_{\pmb{\theta}}\left(\pmb{x}^{(m)}+\sigma\pmb{\epsilon}^{(m)}\right)+\frac{\pmb{\epsilon}^{(m)}}{\sigma}\|^2,\quad\pmb{\epsilon}^{(m)}\sim\mathcal{N}\left(\pmb{0},\mathrm{\pmb{I}}\right).
\end{align}`
...
`$$\mathcal{l}(\pmb{\theta};\sigma_i)=\mathbb{E}_{p(\pmb{x})}\left[\frac{1}{2}\|\pmb{s}_{\pmb{\theta}}(\pmb{x}+\sigma_i\pmb{\epsilon})+\frac{\pmb{\epsilon}}{\sigma_i}\|^2\right].$$`
