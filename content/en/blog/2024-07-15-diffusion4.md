...
here `$\alpha_i=\xi\sigma_i^2/\sigma_L^2$` is the step size and `$\pmb{s}_{\pmb{\theta}}(\pmb{x}_t,\sigma_i)$` denotes the score estimator for noise level `$\sigma_i$`. Iteration over `$t$` is repeated sequentially for each `$\sigma_i,\ i=1,\cdots,L$`.
