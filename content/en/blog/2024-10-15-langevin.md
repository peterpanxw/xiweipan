...
Unlike in physics, the constants involved here can either be omitted or specified arbitrarily for sampling algorithms. If we choose `$\sigma=\sqrt{2/\tau}$`, Equation `$\eqref{eq9}$` becomes "Equation (1)" in [this blog](https://xiweipan.com/en/2024/07/15/diffusion-model-score/):
