...
`$$\mathrm{arg max}\limits{\pmb{\phi}, \pmb{\theta}} \mathbb{E}_{q_{\pmb{\phi}}(\pmb{z}|\pmb{x})}\left[log\,p_{\pmb{\theta}}(\pmb{x}|\pmb{z})\right]-D_{\mathrm{KL}}\left(q_{\pmb{\phi}}(\pmb{z}|\pmb{x})\|p(\pmb{z})\right)\approx\mathrm{arg max}\limits{\pmb{\phi}, \pmb{\theta}}\sum_{l=1}^Llog\,p_{\pmb{\theta}}(\pmb{x}|\pmb{z}^{(l)})-D_{\mathrm{KL}}\left(q_{\pmb{\phi}}(\pmb{z}|\pmb{x})\|p(\pmb{z})\right), \tag{6}$$`
