...
&\approx\mathrm{arg max}\limits{\pmb{\phi}, \pmb{\theta}}\sum_{l=1}^L log\,p_{\pmb{\theta}}(\pmb{x}|\pmb{z}^{(l)})-D_{\mathrm{KL}}\left(q_{\pmb{\phi}}(\pmb{z}|\pmb{x})\|p(\pmb{z})\right), \tag{6}
\end{align}`
where latents `$\left\lbrace\pmb{z}^{(l)}\right\rbrace_{l=1}^L$` are sampled from `$q_{\pmb{\phi}}(\pmb{z}|\pmb{x})$` for every observation `$\pmb{x}$` in the dataset.